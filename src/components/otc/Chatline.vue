    ...
    <div>
        <div class="chat-line">
            <div class="scroll-chat" id="scrollChat">
                <div class="chat-container-box" id="sysbox">
                    <div class="system-box">
                        <p class="msg-content" v-show="msg.status==1">{{$t('otc.orderStatus1')}}</p>
                        <p class="msg-content" v-show="msg.status==2">{{$t('otc.orderStatus2')}}</p>
                        <p class="msg-content" v-show="msg.status==3">{{$t('otc.orderStatus3')}}</p>
                        <p class="msg-content" v-show="msg.status==4">{{$t('otc.orderStatus4')}}</p>
                        <p class="msg-content" v-show="msg.status==0">{{$t('otc.orderStatus0')}}</p>
                    </div>
                </div>
                <h5 class="more" v-show="currentPage<totalPage||currentPage==totalPage">
                    <Icon type="clock"></Icon>
                    <span @click="getBefore">{{$t('otc.loadMore')}}</span>
                </h5>
            ...
            <div class="send-msg-box">
                <div class="img-btn" style="cursor: default;">
                    <!-- <Icon type="image" class="img-icons"></Icon>
                                            <input title="上传图片" type="file" accept="image/jpg,image/jpeg,image/png" class="input-file"> -->
                </div>
                <input v-model="mytext" @keyup.enter="sendName" autocomplete="off" type="text" :placeholder="$t('otc.sendTips')" style=" background-attachment: scroll; background-size: 16px 18px; background-position: 98% 50%;">
    ...
    methods: {
                    ...
                    self.msgLists.push(self.otheritem)
                     ...
                     if(self.fOpenNotice && window.Notification && Notification.permission == "granted") {
                        var notification = new Notification(self.msg.otherSide+"：", {
                            ...
                            icon: 'https://www.caymanex.pro/assets/images/btcicon.png'
                        });
        ...
        sendName: function() {
            if (this.mytext) {
                ...
                this.$Message.info(this.$t('otc.sendTipsRule'));
            }
        },
        CurentTime: function() {
            var now = new Date();
