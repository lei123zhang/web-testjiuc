            ...
            <div class="col-xs-12 rightarea-con">
                <div class="user-top-icon">
                    ...
                    <Row class="user-right">
                        <Col span="8">
                        <i class="m3"></i>
                        <div class="itp" v-if="user.realVerified==0&&user.phoneVerified==0&&user.fundsVerified==0">{{$t('uc.safeLevelLow')}}</div>
                        <div class="itp" v-else-if="user.realVerified==1&&user.phoneVerified==1&&user.fundsVerified==1">{{$t('uc.safeLevelHigh')}}</div>
                        <div class="itp" v-else>{{$t('uc.safeLevelMiddle')}}</div>
                        </Col>                                                                                                                                                                         
                    </Row>
                </div>
                    ...
                    <div class="account-in">
                        ...
                        <div class="account-item" style="display:none">
                            <div class="account-item-in">
                                <Icon type="person" style="font-size: 18px;color: #FF8B1A;"></Icon>
                        ...
                        <div class="account-item">
                            <div class="account-item-in">
                                <Icon type="card" style="font-size: 18px;color: #FF8B1A;"></Icon>
                                <span class="card-number">{{$t('uc.realnameAuthentication')}}</span>
                                <p v-if="user.realVerified==1" class="bankInfo" style="color: grey;">{{user.realName}}</p>
                                <p v-else class="bankInfo" style="color: grey;">
                                    {{$t('uc.noRealname')}}
                                </p>
                                <span v-if="user.realVerified==1">{{$t('uc.bound')}}</span>
                                <span v-else-if="user.realAuditing==1">{{$t('uc.auditing')}}</span>
                                <a v-else class="btn" @click="showItem(6)">{{$t('uc.bind')}}</a>
                            </div>
                            <div class="account-detail" v-show="choseItem==6">
                                <div class="detail-list" style="width: 100%;">
                                    <Form ref="formValidate6" :model="formValidate6" :rules="ruleValidate" :label-width="85">
                                        <!-- 真实姓名 -->
                                        <FormItem :label="$t('uc.realName')" prop="realName">
                                            <Input v-model="formValidate6.realName" size="large"/>
                                        </FormItem>
                                        <!-- 身份证号 -->
                                        <FormItem :label="$t('uc.idCard')" prop="idCard">
                                            <Input v-model="formValidate6.idCard" size="large"/>
                                        </FormItem>
                                        <div style="height:350px;">
                                            <Col span="8">
                                                <input type="hidden" name="imgPreview" :value="imgPreview" />
                                                <div>{{$t('uc.imgPreview')}}</div>
                                        ...
                                        <!-- Button -->
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate6')" style="margin-left: -85px">{{$t('common.save')}}</Button>
                                            <Button type="ghost" @click="handleReset('formValidate6')" style="margin-left: 8px">{{$t('common.reset')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                            </div>
                        ...
                        <div class="account-item">
                            <div class="account-item-in">
                                <Icon type="email" style="font-size: 18px;color: #FF8B1A;"></Icon>
                                <span class="card-number">{{$t('common.email')}}</span>
                                ...
                                <span v-if="user.emailVerified==1">{{$t('uc.bound')}}</span>
                                <a v-else class="btn" @click="showItem(2)">{{$t('uc.bind')}}</a>
                            </div>
                            <div class="account-detail" v-show="choseItem==2">
                                <div class="detail-list">
                                    <Form ref="formValidate2" :model="formValidate2" :rules="ruleValidate" :label-width="110">
                                        <!-- mail -->
                                        <FormItem :label="$t('common.email')" prop="mail">
                                            <Input v-model="formValidate2.mail" size="large"/>
                                        </FormItem>
                                        <!-- 登录密码 -->
                                        <FormItem :label="$t('common.loginPwd')" prop="password">
                                            <Input v-model="formValidate2.password" size="large" type="password"/>
                                        </FormItem>
                                        <!-- 邮箱验证码 -->
                                        <FormItem :label="$t('common.emailCode')" prop="vailCode1">
                                            ...
                                            <div class="timebox" slot="append">
                                                <Button @click="send(1)" :disabled="sendMsgDisabled1">
                                                    <span v-if="sendMsgDisabled1">{{time1+$t('common.second')}}</span>
                                        ...
                                        <!-- Button -->
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate2')">{{$t('common.save')}}</Button>
                                            <Button type="ghost" @click="handleReset('formValidate2')" style="margin-left: 8px">{{$t('common.reset')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                            </div>
                        ...
                        <div class="account-item">
                            <div class="account-item-in">
                                <Icon type="ios-telephone" style="font-size: 18px;color: #FF8B1A;"></Icon>
                                <span class="card-number">{{$t('common.phone')}}</span>
                                ...
                                <span v-if="user.phoneVerified==1">{{$t('uc.bound')}}</span>
                                <a v-else class="btn" @click="showItem(3)">{{$t('uc.bind')}}</a>
                            </div>
                            <div class="account-detail" v-show="choseItem==3">
                                <div class="detail-list">
                                    <Form ref="formValidate3" :model="formValidate3" :rules="ruleValidate" :label-width="110">
                                        <!-- 手机 -->
                                        <FormItem :label="$t('common.phone')" prop="mobile">
                                            <Input v-model="formValidate3.mobile" size="large"/>
                                        </FormItem>
                                        <!-- 登录密码 -->
                                        <FormItem :label="$t('common.loginPwd')" prop="password">
                                            <Input v-model="formValidate3.password" size="large" type="password"/>
                                        </FormItem>
                                        <!-- 手机验证码 -->
                                        <FormItem :label="$t('common.phoneCode')" prop="vailCode2">
                                            <Input v-model="formValidate3.vailCode2" size="large">
                                            <!-- <Button slot="append">点击获取</Button> -->
                                            <div class="timebox" slot="append">
                                                <Button @click="send(2)" :disabled="sendMsgDisabled2">
                                                    <span v-if="sendMsgDisabled2">{{time2+$t('common.second')}}</span>
                                        ...
                                        <FormItem>
                                            ...
                                            <Button type="ghost" @click="handleReset('formValidate3')" style="margin-left: 8px">{{$t('common.reset')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                            </div>
                        ...
                        <div class="account-item">
                            <div class="account-item-in">
                                <Icon type="locked" style="font-size: 18px;color: #FF8B1A;"></Icon>
                                <span class="card-number">{{$t('common.loginPwd')}}</span>
                                <p class="bankInfo" style="color: grey;">
                                    {{$t('uc.loginPwdTip')}}
                                </p>

                                <a class="btn" v-if="user.phoneVerified==0" @click="noPhone">{{$t('common.modify')}}</a>
                                <a class="btn" v-else @click="showItem(4)">{{$t('common.modify')}}</a>
                            </div>
                            <div class="account-detail" v-show="choseItem==4">
                                <div class="detail-list">
                                    <Form ref="formValidate4" :model="formValidate4" :rules="ruleValidate" :label-width="90">
                                        <!-- oldPw -->
                                        <FormItem :label="$t('common.oldLoginPw')" prop="oldPw">
                                            <Input v-model="formValidate4.oldPw" size="large" type="password"/>
                                        </FormItem>
                                        <!-- newPw -->
                                        <FormItem :label="$t('common.newLoginPw')" prop="newPw">
                                            <Input v-model="formValidate4.newPw" size="large" type="password"/>
                                        </FormItem>
                                        <!-- newPwConfirm -->
                                        <FormItem :label="$t('common.newPwConfirm')" prop="newPwConfirm">
                                            <Input v-model="formValidate4.newPwConfirm" size="large" type="password"/>
                                        </FormItem>
                                        <!-- 手机验证码 -->
                                        <FormItem :label="$t('common.phoneCode')" prop="vailCode3">
                                            <Input v-model="formValidate4.vailCode3" size="large">
                                            <!-- <Button slot="append">点击获取</Button> -->
                                            <div class="timebox" slot="append">
                                                <Button @click="send(3)" :disabled="sendMsgDisabled3">
                                                    <span v-if="sendMsgDisabled3">{{time3+$t('common.second')}}</span>
                                                    <span v-if="!sendMsgDisabled3">{{$t('common.getCode')}}</span>
                                                </Button>
                                            </div>
                                            </Input>
                                        </FormItem>
                                        <!-- Button -->
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate4')">{{$t('common.save')}}</Button>
                                            <Button type="ghost" @click="handleReset('formValidate4')" style="margin-left: 8px">{{$t('common.reset')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                            </div>
                        ...
                        <div class="account-item">
                            <div class="account-item-in">
                                <Icon type="lock-combination" style="font-size: 18px;color: #FF8B1A;"></Icon>
                                <span class="card-number">{{$t('common.priceW')}}</span>
                                <p class="bankInfo" style="color: grey;">
                                    {{$t('uc.priceWTip')}}
                                </p>
                                <a class="btn" v-if="user.fundsVerified!=1" @click="showItem(5)">{{$t('common.setting')}}</a>
                                <a class="btn" v-else @click="showItem(5)">{{$t('common.modify')}}</a>
                            </div>
                            <div class="account-detail" v-show="choseItem==5">
                                <!-- 设置 -->
                                <div class="detail-list" v-show="user.fundsVerified!=1">
                                    <Form ref="formValidate7" :model="formValidate7" :rules="ruleValidate" :label-width="90">
                                        <!-- newMPw -->
                                        <FormItem :label="$t('common.priceW')" prop="pw7">
                                            <Input v-model="formValidate7.pw7" size="large" type="password"/></Input>
                                        </FormItem>
                                        <!-- newMPwConfirm -->
                                        <FormItem :label="$t('common.repassword')" prop="pw7Confirm">
                                            <Input v-model="formValidate7.pw7Confirm" size="large" type="password"/></Input>
                                        </FormItem>

                                        <!-- Button -->
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate7')">{{$t('common.save')}}</Button>
                                            <Button type="ghost" @click="handleReset('formValidate7')" style="margin-left: 8px">{{$t('common.reset')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                                <!-- 修改 -->
                                <div class="detail-list" v-show="user.fundsVerified==1 && !fGetBackFundpwd">
                                    <Form ref="formValidate5" :model="formValidate5" :rules="ruleValidate" :label-width="90">
                                        <!-- oldPw -->
                                        <FormItem :label="$t('common.oldPriceW')" prop="oldPw">
                                            <Input v-model="formValidate5.oldPw" size="large" type="password"/>
                                        </FormItem>
                                        <!-- newMPw -->
                                        <FormItem :label="$t('common.newPriceW')" prop="newMPw">
                                            <Input v-model="formValidate5.newMPw" size="large" type="password"/>
                                        </FormItem>
                                        <!-- newMPwConfirm -->
                                        <FormItem :label="$t('common.newPwConfirm')" prop="newMPwConfirm">
                                            <Input v-model="formValidate5.newMPwConfirm" size="large" type="password"/>
                                        </FormItem>
                                        <!-- 邮箱验证码 -->
                                        <!-- <FormItem :label="$t('common.phoneCode')" prop="vailCode5">
                                            <Input v-model="formValidate5.vailCode5" size="large">
                                            <!-- <Button slot="append">点击获取</Button>
                                            <div class="timebox" slot="append">
                                                <Button @click="send(5)" :disabled="sendMsgDisabled5">
                                                    <span v-if="sendMsgDisabled5">{{time5+$t('common.second')}}</span>
                                                    <span v-if="!sendMsgDisabled5">{{$t('common.getCode')}}</span>
                                                </Button>
                                            </div>
                                            </Input>
                                        </FormItem> -->
                                        ...
                                        <!-- Button -->
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate5')">{{$t('common.save')}}</Button>
                                            <Button type="ghost" @click="handleReset('formValidate5')" style="margin-left: 8px">{{$t('common.reset')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
...
export default {
    ...
    data() {
        const validatePass = (rule, value, callback) => {
            if (value === '') {
                callback(new Error(this.$t('uc.newLoginPwRule')));
        ...
        const validatePassCheck = (rule, value, callback) => {
            if (value === '') {
                callback(new Error(this.$t('uc.newPwConfirmRule1')));
            } //else if (!/^\d+$/.test(value))
             else if (!/([a-zA-Z0-9]){6,18}/.test(value))  {
                callback(new Error(this.$t('uc.newPwConfirmRule1')));
            } else if (value !== this.formValidate4.newPw) {
                callback(new Error(this.$t('uc.newPwConfirmRule2')));
            } else {
                callback();
            }
        };
        const validateMPass = (rule, value, callback) => {
            if (value === '') {
                callback(new Error(this.$t('uc.oldPriceWRule')));
            } //else if (!/^\d+$/.test(value)) 
            else if (!/([a-zA-Z0-9]){6,18}/.test(value)){
                callback(new Error(this.$t('uc.oldPriceWRule')));
            } else {
        ...
        const validateMPassCheck = (rule, value, callback) => {
            if (value === '') {
                callback(new Error(this.$t('uc.newPriceWRule1')));
            }// else if (!/^\d+$/.test(value)) 
            else if (!/([a-zA-Z0-9]){6,18}/.test(value)){
                callback(new Error(this.$t('uc.newPriceWRule1')));
            } else if (value !== this.formValidate5.newMPw) {
                callback(new Error(this.$t('uc.newPriceWRule2')));
            } else {
                callback();
            }
        };
        ...
        return {
            ...
            ruleValidate: {
                mail: [
                    { required: true, type: 'email', message: this.$t('common.emailRule1'), trigger: 'blur' },
                ],
                vailCode1: [
                    { required: true, message: this.$t('common.codeRule'), trigger: 'blur' }
                ],
                mobile: [
                    { required: true, message: this.$t('common.phoneRule'), trigger: 'blur' }
                ],
                vailCode2: [
                    { required: true, message: this.$t('common.codeRule'), trigger: 'blur' }
                ],
                ...
                password: [
                    { required: true, type: 'string', min: 6, message: this.$t('common.passwordRule'), trigger: 'blur' },
                ],
                oldPw: [
                    { required: true, type: 'string', min: 6, message: this.$t('common.oldPasswordRule'), trigger: 'blur' }
                ],
                newPw: [
                    { required: true, type: 'string', min: 6, message: this.$t('common.newPasswordRule'), trigger: 'blur' },
                    { validator: validatePass, trigger: 'blur' },
                ],
                newPwConfirm: [
                    { required: true, type: 'string', min: 6, message: this.$t('uc.newPwConfirmRule1'), trigger: 'blur' },
                ...
                newMPwConfirm: [
                    { required: true, type: 'string', min: 6, message: this.$t('uc.newPriceWRule1'), trigger: 'blur' },
                ...
                realName: [
                    { required: true, message: this.$t('uc.realNameRule'), trigger: 'blur' }
                ],
                idCard: [
                    { required: true, message: this.$t('uc.idCardRule'), trigger: 'blur' }
                ],
                 newMPw8: [
    ...
    methods: {
        ...
        noPhone() {
            this.$Message.info(this.$t('uc.bindPhoneRule'));
        },
        showItemFundpwd() {
        ...
        submit(name) {
            ...
            if (name == 'formValidate6') {
                if(this.imgPreview == "") {
                    this.$Message.error(this.$t('uc.imgPreviewRule'));
                    return false;
                }
                if(this.imgNext == "") {
                    this.$Message.error(this.$t('uc.imgNextRule'));
                    return false;
                }
                if(this.imgLast == "") {
                    this.$Message.error(this.$t('uc.imgLastRule'));
                    return false;
                }
                let param = {}
...
<style scoped>
#frontCardImg,#backCardImg,#handCardImg{
    width: 270px;
    max-height:175px;
}
